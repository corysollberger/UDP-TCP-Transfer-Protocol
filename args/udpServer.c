...
void* serverTCP(){
	...
	while(buffer[2] != 'L'){
		...
		if (s > 0){ //Ack Received
			...
			fseek(fp, 46*(count), SEEK_SET);
			sem_post(&ACKS);
		}
	}
	printf("Buffer: %s\n", buffer);
	int n = write(socket2,"ALL", 4);
	if(n<0){
		printf("ERROR: Problem writing to socket\n");
	}
	printf("DONE\n");
	done = 1;
	close(socket2);
...
void* serverUDP(){
	...
	me.sin_port = htons(udpServerPortNum);
	inet_pton(AF_INET, udpServerIp, &(me.sin_addr));
	...
	while(1){
		...
		while (done == 0){
			...
			int nums = 0;
			val = fread((buffMe+4), sizeof(char), (50-4), fp);
